...
def possible_syscall(filename="system_call.txt"):
	f=open(filename,'r')
	global data_index
	i=0
	for line in f:
...
def calculate_series(set_sequence,slength=6):
	num_data=list()#numerical data_index
	global data_index
	for batches in set_sequence:
		l=[0]*slength
		...
		for systemcall in batches:
			try:
				l[i]=data_index["syscall"][systemcall]+1
				
			except:
				l[i]=data_index["syscall"]["others"]+1
			i+=1
		num_data.append(l)
	return(num_data)
...
if(__name__=="__main__"):
	...
	# for i in range(len(set_sequence)):
		# print(set_sequence[i],dt[i],"\n")
	num_data1=calculate_series(set_sequence1,6)
	num_data2=calculate_series(set_sequence2,6)
	# for ele in num_data:
		# print(ele)
	# print("\n")
	# matrix=np.matrix(num_data)
	# print(matrix)
	# print(num_data)
	write_into_file("output1_series.txt",num_data1)
	write_into_file("output2_series.txt",num_data2)